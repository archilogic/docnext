...
public enum CoreImageDirection {
    ...
    boolean shouldChangeToNext( final CoreImageState engine ) {
        ...
        case L2R:
            return engine.matrix.tx < engine.surfaceSize.width - engine.surfaceSize.width / PAGE_CHANGE_THREASHOLD - engine.pageSize.width
                    * engine.matrix.scale * Device.pagePerDisplay() - engine.getHorizontalPadding() * 2;
        case R2L:
    ...
    void updateOffset( final CoreImageState engine , final boolean isNext , final int page ) {
        final int sign = ( this == L2R || this == T2B ) ^ isNext ? -1 : 1;

        switch ( this ) {
        case L2R:
        case R2L:
            engine.matrix.tx += sign * engine.pageSize.width * engine.matrix.scale * page;
            break;
        case T2B:
        case B2T:
            engine.matrix.ty += sign * engine.pageSize.height * engine.matrix.scale * page;
            break;
        default:
            throw new RuntimeException();
        }
