...
import jp.archilogic.docnext.android.coreview.image.PageInfo;
import jp.archilogic.docnext.android.coreview.image.PageInfo.PageTextureStatus;
import jp.archilogic.docnext.android.coreview.image.TextureInfo;
...
public class CoreImageRenderEngine {
    ...
    private Point margin( final CoreImageMatrix matrix , final CoreImageState state , final int page , final int deltaPage ) {
        ...
        final float margin = state.getHorizontalMargin();

        for ( int i = -1 ; i >= deltaPage ; i-- ) {
            if ( !state.facingFirstPages.contains( page + i - deltaPage ) ) {
                deltaPoint.x -= margin * xSign;
            }
        }

        for ( int i = 1 ; i <= deltaPage ; i++ ) {
            if ( !state.facingFirstPages.contains( page + i - deltaPage - 1 ) ) {
                deltaPoint.x += margin * xSign;
            }
        }
